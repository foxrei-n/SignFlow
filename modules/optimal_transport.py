...
class OTPlanSampler:
    ...
    def sample_plan_with_multi_labels(self, x0, x1, y0=None, y1=None, replace=True):
        pi = self.get_map(x0, x1)
        i, j = self.sample_map(pi, x0.shape[0], replace=replace)
        return (
            x0[i],
            x1[j],
            [y[i] for y in y0] if y0 is not None else None,
            [y[j] for y in y1] if y1 is not None else None,
        )
